title: redis4.0_skiplist源码剖析
category: redis 
date: 2017-12-02
tags: [redis4.0,skiplist,sourcecode]
toc: false
comments: false
---
...
zskiplistNode *zslInsert(zskiplist *zsl, double score, sds ele) {
    ...
    //创建一个新的节点，用来记录else score
    level = zslRandomLevel();
    if (level > zsl->level) {
        for (i = zsl->level; i < level; i++) {
            rank[i] = 0;
            update[i] = zsl->header;
	    //到最后一个节点所指向的NULL的距离为length
